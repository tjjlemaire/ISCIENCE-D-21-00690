...
from MorphoSONIC.sources import PlanarDiskTransducerSource, ExtracellularCurrent
...
phasevec = np.linspace(-2e-3, 2e-3, nperslice)  # m
fontsize = 10
...
def getPacField(source, x, y, z):
    ...
    fpath = os.path.join(fieldsroot, fname)
...
if __name__ == '__main__':
    ...
    for r in radii:
        for Fdrive in freqs:
            ...
            axes['b2'].plot(z / dfocal, amps_z / amps_z.max(), c=color, clip_on=False)

            zphase = phasevec + dfocal
            zphase = zphase[zphase > 0]
            Pac_z = getPacField(source, 0, 0, zphase)
            phases_z = np.angle(Pac_z)  # rad
            y = np.unwrap(phases_z) / np.pi
            phasefocal = np.interp(dfocal, zphase, y)
            y -= phasefocal
            max_phase_shift = max(np.nanmax(np.abs(y)), max_phase_shift)
            axes['b4'].plot((zphase - dfocal) * 1e3, y, c=color, clip_on=False)
