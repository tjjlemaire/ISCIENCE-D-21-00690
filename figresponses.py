...
responseroot = getSubRoot('responses')
...
if __name__ == '__main__':
    ...
    for i, (k, fiber) in enumerate(fibers.items()):
        ...
        fpath = fiber.simAndSave(source, pps[k], outputdir=responseroot, overwrite=False)
        data, meta = loadData(fpath)

        df = data[fiber.central_ID]
        currents = fiber.getCurrentsDict(df)
        tthr = timeThreshold(
            df['t'].values, df['Qm'].values / fiber.pneuron.Cm0 * V_TO_MV, dQnorm_thr)
        ...
        for ax, varkey in zip(axes, ['Cm', 'Vm', 'Qm']):
            ...
            ax.set_ylabel(
                ylabel.replace('_', '').replace('^', ''), fontsize=fontsize, labelpad=ypad)
